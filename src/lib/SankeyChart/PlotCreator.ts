...
export class PlotCreator {
  ...
  private calculateNodeHeightUnit(): void {
    ...
    for (let column of this.nodes_data_structure) {
      let column_sum = column.reduce((sum: number, node: any) => {
        return (
          sum + Math.max(node.left_side_sum || 0, node.right_side_sum || 0)
        );
      }, 0);
      max_column_sum = Math.max(max_column_sum, column_sum);
    }
    this.node_height_unit =
      ((1 - (this.settings.vertical_gap_between_nodes || 0)) *
        this.plot_height) /
      max_column_sum;
  }
    ...
    for (let i in this.links_data_structure) {
      ...
      if (from_column === to_column) {
        ...
        if (!link.hasOwnProperty("link_type")) {
          ...
          ["LR", "RL", "LL", "RR"].includes(link["link_type"] as LinkType)
        ) {
          link["link_type"] = ((link["link_type"] as string)[1] +
            (link["link_type"] as string)[0]) as LinkType;
          (link as any)["reversed"] = true;
          let _ = link["from"];
          link["from"] = link["to"];
          link["to"] = _;
  ...
  private sortLinks(): void {
    this.links_data_structure.sort(
      (a, b) => a["from"]["node"] - b["from"]["node"],
    );
    this.links_data_structure.sort(
      (a, b) => a["from"]["column"] - b["from"]["column"],
    );
  ...
  private prepareData(): void {
    ...
    this.node_height_unit =
      ((1 - this.settings.vertical_gap_between_nodes!) * this.plot_height) /
      max_column_sum;

    for (let i in this.nodes_data_structure) {
      let gaps = this.nodes_data_structure[i].length + 1;
      let gap_height =
        ((this.settings.vertical_gap_between_nodes! * this.plot_height) /
          this.node_height_unit +
          (max_column_sum - col_sums[parseInt(i)])) /
        gaps;
    ...
    for (let column_index in this.nodes_data_structure) {
      for (let node_index in this.nodes_data_structure[column_index]) {
        ...
        left_side_links_special.sort(
          (a, b) => b["to"]["node"] - a["to"]["node"],
        );
        left_side_links_special.sort(
          (a, b) => b["to"]["column"] - a["to"]["column"],
        );
        let node_height = node["height"];
        let left_side_curve =
          -this.settings.link_arc_iterated_increase! +
          this.settings.link_min_arc!;
        ...
        for (let i = left_side_links_special.length - 1; i >= 0; i--) {
          ...
          special_link["left_side_curve"] =
            left_side_curve + this.settings.link_arc_iterated_increase!;
          left_side_curve +=
            link_height + this.settings.link_arc_iterated_increase!;
        }

        right_side_links_special.sort(
          (a, b) => b["from"]["node"] - a["from"]["node"],
        );
        right_side_links_special.sort(
          (a, b) => a["from"]["column"] - b["from"]["column"],
        );
        ...
        for (let i = right_side_links_special.length - 1; i >= 0; i--) {
          ...
          special_link["right_side_curve"] =
            right_side_curve + this.settings.link_arc_iterated_increase!;
          right_side_curve +=
            link_height + this.settings.link_arc_iterated_increase!;
        }
      }
  ...
  public changeColumnRange(first_column: number, last_column: number): boolean {
    if (
      first_column < this.settings.start_column_count_from! ||
      last_column >
        this.nodes_data_structure.length +
          this.settings.start_column_count_from! ||
  ...
  public removePlot(): void {
    if (this.sankey_plot) {
      ...
      if (
        this.sankey_plot.dom_node &&
        this.dom_container.contains(this.sankey_plot.dom_node)
      ) {
        this.dom_container.removeChild(this.sankey_plot.dom_node);
      }
    }
