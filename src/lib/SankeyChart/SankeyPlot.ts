...
import { SankeyLinkRL, SankeyLinkLR, SankeyLinkLL, SankeyLinkRR } from './SankeyLinks';

import { SVGLineAttributes, defaultLineAttributes, ColumnNamesStyle } from './types';
...
export class SankeyPlot {
  ...
  private on_node_click_function: ((node_info: any, node_data_reference: any, node_element: SVGElement, event: MouseEvent) => void) | undefined;
  private on_link_click_function: ((link_info: any, link_data_reference: any, link_element: SVGElement, event: MouseEvent) => void) | undefined;
  private on_node_hover_function: (node_info: any, node_data_reference: any, node_element: SVGElement, event: MouseEvent) => string;
  private on_link_hover_function: (link_info: any, link_data_reference: any, link_element: SVGElement, event: MouseEvent) => string;
  ...
  constructor(
    ...
    on_node_click_function: ((node_info: any, node_data_reference: any, node_element: SVGElement, event: MouseEvent) => void) | undefined = undefined,
    on_link_click_function: ((link_info: any, link_data_reference: any, link_element: SVGElement, event: MouseEvent) => void) | undefined = undefined,
    on_node_hover_function: (node_info: any, node_data_reference: any, node_element: SVGElement, event: MouseEvent) => string = (node_info) => `${node_info['label']}`,
    on_link_hover_function: (link_info: any, link_data_reference: any, link_element: SVGElement, event: MouseEvent) => string = (link_info) => `From: (column - ${link_info['from_column']}, node - ${link_info['from_node']}, label - "${link_info['from_label']}") To: (column - ${link_info['to_column']}, node - ${link_info['to_node']}, label - "${link_info['to_label']}") Value: ${link_info['value']}`,
    ...
    this.column_width = this.plot_width / (this.last_column - this.first_column);
    ...
    this.line_definition_object = { ...defaultLineAttributes, ...line_definition_object };
    ...
    if (this.dom_node) {
      this.dom_node.addEventListener('click', (e: MouseEvent) => {
          ...
          if (this.on_node_click_function) {
            ...
            if (column !== null && position !== null) {
              ...
              if (this.nodes[columnIndex] && this.nodes[columnIndex][positionIndex]) {
                ...
                const node_data = this.nodes_data_structure[selected_node.column][selected_node.position]['properties_object'];
                this.on_node_click_function(this.getNodeInfo(e), node_data, target, e);
              } else {
        ...
        if (target.getAttribute('identifier') === 'SankeyLink') {
          if (this.on_link_click_function) {
            ...
            const link_data = this.links_data[parseInt(target.getAttribute('links_array_id')!)];
            this.on_link_click_function(this.getLinkInfo(e), link_data, link_div, e);
          }
        }
      });
      ...
      this.dom_node.addEventListener('pointermove', (e: PointerEvent) => {
        if (this.selected_node && this.selected_node.mouse_locked) {
          this.selected_node.move(e.clientX, e.clientY - this.dom_node.getBoundingClientRect().top);
        }
      });
        ...
        if (target.getAttribute('identifier') === 'SankeyNode') {
            ...
            if (column !== null && position !== null) {
              ...
              if (this.nodes[columnIndex] && this.nodes[columnIndex][positionIndex]) {
                ...
                if (this.on_node_hover_function) {
                  const div_ = target;
                  const snode_ = this.nodes[parseInt(div_.getAttribute('column')!)][parseInt(div_.getAttribute('position')!)];
                  const node_d_ = this.nodes_data_structure[snode_.column][snode_.position]['properties_object'];
                  text_content = this.on_node_hover_function(this.getNodeInfo(e), node_d_, div_, e);
                }
                ...
                const potential_y_coord = Math.max(this.mouseovered_node.current_y - node_info_div.offsetHeight + this.dom_container.getBoundingClientRect().top - 5, 0);
                let potential_x_coord = Math.max(this.mouseovered_node.current_x + this.dom_container.getBoundingClientRect().left + (this.mouseovered_node.width - node_info_div.offsetWidth) / 2, 0);
          ...
          if (!this.selected_node) {
            ...
            if (this.on_link_hover_function) {
              const link_div = target;
              const link_data = this.links_data[parseInt(target.getAttribute('links_array_id')!)];
              text_content = this.on_link_hover_function(this.getLinkInfo(e), link_data, link_div, e);
            }
            ...
            const potential_y_coord = Math.max(e.clientY - link_info_div.offsetHeight - 15, 0);
  ...
  private createLines(): void {
    for (let i = 0; i < this.last_column - this.first_column - 1; i++) {
      ...
      if (this.line_definition_object.stroke || defaultLineAttributes.stroke) {
        attributes.stroke = this.line_definition_object.stroke || defaultLineAttributes.stroke;
      }
      if (this.line_definition_object['stroke-width'] || defaultLineAttributes['stroke-width']) {
        attributes['stroke-width'] = this.line_definition_object['stroke-width'] || defaultLineAttributes['stroke-width'];
      }
      if (this.line_definition_object['stroke-opacity'] || defaultLineAttributes['stroke-opacity']) {
        attributes['stroke-opacity'] = this.line_definition_object['stroke-opacity'] || defaultLineAttributes['stroke-opacity'];
      }
      if (this.line_definition_object['stroke-dasharray'] || defaultLineAttributes['stroke-dasharray']) {
        attributes['stroke-dasharray'] = this.line_definition_object['stroke-dasharray'] || defaultLineAttributes['stroke-dasharray'];
      }
  ...
  private createColumnNames(): void {
    for (let i = 0; i < this.last_column - this.first_column; i++) {
      ...
      if (this.column_names) {
        ...
        number.textContent = (i + this.first_column + this.start_column_count_from).toString();
      }
  ...
  private createNodes(): void {
    ...
    for (let column_index = this.first_column; column_index < this.last_column; column_index++) {
      for (let node_index in this.nodes_data_structure[column_index]) {
        for (let link_index in this.nodes_data_structure[column_index][node_index]['links_references']) {
          let link_ref = this.nodes_data_structure[column_index][node_index]['links_references'][link_index];
          this.links_to_create_indexes.push(link_ref['link_id']);
          if (link_ref['second_node_column'] < this.first_column || link_ref['second_node_column'] >= this.last_column) {
    ...
    if (this.show_links_out_of_range) {
      ...
      let check_range = number_of_columns - this.last_column > this.first_column ?
        [Math.max(this.first_column - this.longest_link_length, 0), this.first_column] :
        [this.last_column, Math.min(this.last_column + this.longest_link_length, number_of_columns)];
    ...
    this.links_to_create_indexes = this.links_to_create_indexes.filter((value, index, self) =>
  ...
  private createLinks(): void {
    ...
    for (let i in this.links_to_create_indexes) {
      ...
      let link_type = link.hasOwnProperty('link_type') ? link['link_type'] : 'RL';
      ...
      switch (link_type) {
        case 'RL':
          new SankeyLinkRL(start_node, end_node, height, this, this.links_to_create_indexes[i], link_color);
          break;
        case 'LR':
          new SankeyLinkLR(start_node, end_node, height, this, this.links_to_create_indexes[i], link_color);
          break;
        case 'LL':
          new SankeyLinkLL(start_node, end_node, height, this, this.links_to_create_indexes[i], link_color);
          break;
        case 'RR':
          new SankeyLinkRR(start_node, end_node, height, this, this.links_to_create_indexes[i], link_color);
          break;
        default:
      ...
      if (link.hasOwnProperty('sublinks')) {
        for (let sublink_index in link['sublinks']) {
          ...
          link_color = sublink_data.hasOwnProperty('color') ? sublink_data['color'] : undefined;
          switch (link_type) {
            case 'RL':
              new SankeyLinkRL(start_node, end_node, height, this, this.links_to_create_indexes[i], link_color, [sublink_data, parseInt(sublink_index)]);
              break;
            case 'LR':
              new SankeyLinkLR(start_node, end_node, height, this, this.links_to_create_indexes[i], link_color, [sublink_data, parseInt(sublink_index)]);
              break;
            case 'LL':
              new SankeyLinkLL(start_node, end_node, height, this, this.links_to_create_indexes[i], link_color, [sublink_data, parseInt(sublink_index)]);
              break;
            case 'RR':
              new SankeyLinkRR(start_node, end_node, height, this, this.links_to_create_indexes[i], link_color, [sublink_data, parseInt(sublink_index)]);
              break;
            default:
              throw new Error('NOT A VALID LINK TYPE');
          }
  ...
  private setAttributes(element: SVGElement, attributes: Record<string, string | number>): void {
  ...
  private getNodeInfo(e: MouseEvent): any {
    ...
    if (column !== null && position !== null) {
      ...
      if (this.nodes[columnIndex] && this.nodes[columnIndex][positionIndex]) {
        ...
        let node_height = Math.max(node_data['left_side_sum'], node_data['right_side_sum']);
        ...
        for (let i in node_data['links_references']) {
          let link = this.links_data[node_data['links_references'][i]['link_id']];
          let second_node_col = node_data['links_references'][i]['second_node_column'];
  ...
  private getLinkInfo(e: MouseEvent): any {
    ...
    let f_label = this.nodes_data_structure[f_column][f_node]['properties_object']['label'];
    let t_label = this.nodes_data_structure[t_column][t_node]['properties_object']['label'];
