abstract class SankeyLink {
  ...
  constructor(
    ...
    this.node1_render_height = this.sankey_plot_object.links_data[this.links_array_index]['left_side_rel_to_node_height'];
    this.node2_render_height = this.sankey_plot_object.links_data[this.links_array_index]['right_side_rel_to_node_height'];
    ...
    if (sublink_data) {
      ...
      this.height = this.sankey_plot_object.node_height_unit * this.sublink_data['value'];
  ...
  protected abstract calculateD(x1: number, x2: number, y1: number, y2: number, path_height: number, node1_width: number, node2_width: number): string;

  public updateD(): void {
    this.dom_node.setAttribute('d', this.calculateD(
  ...
  private linearGradient(color: string | { start: string; end: string } | undefined): void {
    if (this.sankey_plot_object.linear_gradient_links && !this.fill && !this.sublink_data) {
      let color1, color2;
      ...
      if (!this.sankey_plot_object.linear_gradient_colors_object.hasOwnProperty(color1)) {
        this.sankey_plot_object.linear_gradient_colors_object[color1] = {};
      }
      if (!this.sankey_plot_object.linear_gradient_colors_object[color1].hasOwnProperty(color2)) {
        let linear = document.createElementNS("http://www.w3.org/2000/svg", 'linearGradient');
        ...
        this.sankey_plot_object.linear_gradient_colors_object[color1][color2] = true;
      }
      ...
      this.fill = color;
      this.opacity = this.sublink_data ? this.sankey_plot_object.default_sublinks_opacity : this.sankey_plot_object.default_links_opacity;
    } else {
      // Use default color if no color is provided
      this.fill = this.sublink_data ? this.sankey_plot_object.default_sublinks_color : this.sankey_plot_object.default_links_color;
      this.opacity = this.sublink_data ? this.sankey_plot_object.default_sublinks_opacity : this.sankey_plot_object.default_links_opacity;
    }
  }

  protected createDomElement(): void {
    this.dom_node = document.createElementNS("http://www.w3.org/2000/svg", 'path');
    ...
    if (this.sublink_data) {
      this.dom_node.setAttribute('sublink_index', this.sublink_index!.toString());
    }
    this.sankey_plot_object.dom_node.appendChild(this.dom_node);
  }
...
export class SankeyLinkRL extends SankeyLink {
  constructor(node1: any, node2: any, height: number, sankey_plot_object: any, links_array_index: number, color: string | undefined = undefined, sublink_data: any = undefined) {
    super(node1, node2, height, sankey_plot_object, links_array_index, color, sublink_data);
    this.createDomElement();
  }

  protected calculateD(x1: number, x2: number, y1: number, y2: number, path_height: number, node1_width: number, node2_width: number): string {
    ...
    if ([x1, x2, y1, y2, path_height, node1_width, node2_width].some(isNaN)) {
      console.error('Invalid numerical values in link calculation', { x1, x2, y1, y2, path_height, node1_width, node2_width });
...
export class SankeyLinkLR extends SankeyLink {
  ...
  constructor(node1: any, node2: any, height: number, sankey_plot_object: any, links_array_index: number, color: string | undefined = undefined, sublink_data: any = undefined) {
    super(node1, node2, height, sankey_plot_object, links_array_index, color, sublink_data);
    this.left_side_curve = this.sankey_plot_object.links_data[this.links_array_index]['left_side_curve'];
    this.right_side_curve = this.sankey_plot_object.links_data[this.links_array_index]['right_side_curve'];
    if (this.sublink_data) {
      this.left_side_curve += this.parent_link_height! * (1 - this.sublink_data['shift']) - this.height;
      this.right_side_curve += this.parent_link_height! * (1 - this.sublink_data['shift']) - this.height;
    }
    this.createDomElement();
  }
